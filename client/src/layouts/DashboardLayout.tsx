...
import { LayoutDashboard, Zap, LogOut, Settings, UserCircle } from 'lucide-react';
    ...
    <div className="min-h-screen flex bg-background/50 p-4 gap-6 animate-fade-in">
      <aside className="w-64 glass-panel flex-col justify-between hidden md:flex sticky top-4 h-[calc(100vh-2rem)]">
        ...
        <div className="p-4 mt-auto mb-4 mx-4 border-t border-white/5 pt-6">
          <button onClick={handleLogout} className="flex items-center gap-3 p-3 w-full rounded-xl hover:bg-red-500/10 transition-colors text-gray-400 hover:text-red-400 group cursor-pointer">
      ...
      <main className="flex-1 flex flex-col h-[calc(100vh-2rem)]">
        <header className="glass-panel mb-6 p-4 px-6 flex justify-between items-center z-10 shrink-0">
          ...
          <div className="flex items-center gap-4">
             ...
             <Link to="/dashboard/settings" title="Profile Settings" className="h-10 w-10 bg-gradient-to-tr from-surface to-primary/20 rounded-xl flex items-center justify-center border border-white/10 text-white font-bold shadow-lg cursor-pointer hover:border-primary/50 transition-colors">
